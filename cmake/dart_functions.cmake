...
function(dart_generate_export_header)
  set(prefix dart_generate_export_header)
  ...
  set(target_name ${dart_generate_export_header_TARGET_NAME})
  set(base_name ${dart_generate_export_header_BASE_NAME})
  set(include_dir ${dart_generate_export_header_INCLUDE_DIR})
  set(install_dir ${dart_generate_export_header_INSTALL_DIR})
  set(export_filename ${dart_generate_export_header_EXPORT_FILENAME})
  set(export_all_symbolx_by_default ${dart_generate_export_header_EXPORT_ALL_SYMBOLS_BY_DEFAULT})
  ...
  if(NOT include_dir)
    message(FATAL_ERROR "DEVELOPER ERROR: You must specify INCLUDE_DIR!")
    return()
  endif()
  if(NOT export_filename)
    message(FATAL_ERROR "DEVELOPER ERROR: You must specify EXPORT_FILENAME!")
    return()
  endif()
  ...
  if(NOT TARGET ${target_name})
    message(
      FATAL_ERROR
        "DEVELOPER ERROR: Invalid target \"${target_name}\" is passed! "
  ...
  # Hide symbols by default unless specified not to do so
  if(UNIX AND NOT export_all_symbolx_by_default)
  ...
  # Setup paths
  set(include_export_filepath_detail "${include_dir}/detail/${export_filename}")
  set(export_filepath "${CMAKE_CURRENT_BINARY_DIR}/${export_filename}")
  set(export_filepath_detail "${CMAKE_CURRENT_BINARY_DIR}/detail/${export_filename}")

  # Generate CMake's default export header
  include(GenerateExportHeader)
  generate_export_header(${target_name}
    EXPORT_MACRO_NAME DETAIL_${base_name}_EXPORT
    EXPORT_FILE_NAME ${export_filepath_detail}
  )

  # Generate final export header
  file(
    WRITE ${export_filepath}
    ...
    "    DETAIL_${base_name}_EXPORT\n"
    "\n"
    "#include \"${include_export_filepath_detail}\"\n"
  )

  # Install generated export files
  set(export_install_path "${CMAKE_INSTALL_INCLUDEDIR}/${install_dir}/${include_dir}")
  set(export_install_path_detail "${export_install_path}/detail/")
  install(
    FILES "${export_filepath}"
    DESTINATION "${export_install_path}"
  )
  install(
    FILES "${export_filepath_detail}" 
    DESTINATION "${export_install_path_detail}"
  )
...
function(dart_add_module)
  ...
  dart_generate_export_header(
    BASE_NAME ${PROJECT_NAME}_${module_name}
    TARGET_NAME ${target_name}
    INCLUDE_DIR ${PROJECT_NAME}/${module_name}
    INSTALL_DIR ${PROJECT_NAME}${DART_VERSION_MAJOR}
    EXPORT_FILENAME export.hpp
  )
  ...
  if(dart_add_module_GENERATE_META_HEADER)
    dart_generate_meta_header(
      DESTINATION "all.hpp"
      HEADERS ${headers}
      PREFIX_TO_REMOVE "${project_source_dir}/src/"
  ...
  install(
    ...
    DESTINATION
      ${CMAKE_INSTALL_INCLUDEDIR}/${PROJECT_NAME}${DART_VERSION_MAJOR}/${PROJECT_NAME}/${module_name}
  )
